...
{#if isSelected}
  <div class="space-y-6 p-4">
    <P class="font-semibold" size="lg">Mortgage Calculator</P>
          ...
          {#if hasCalculated}
            <Chart
              options={{
                ...
                xaxis: {
                  categories: balances.map((_, index) => {
                    const quarterNumber = index * 3;
                    const year = Math.floor(quarterNumber / 12);
                    const quarter = Math.floor((quarterNumber % 12) / 3) + 1;
